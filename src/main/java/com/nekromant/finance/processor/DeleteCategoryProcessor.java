...
public class DeleteCategoryProcessor implements CallBackProcessor {
    ...
    public void process(Update update) {
        categoryRepository.deleteCategoryById(Long.parseLong(update.getCallbackQuery().getData().split(" ")[1]));
        categoryRepository.deleteById(Long.parseLong(update.getCallbackQuery().getData().split(" ")[1]));
        messageSender.sendMessage("Категория удалена", String.valueOf(update.getCallbackQuery().getMessage().getChatId()));
    }
